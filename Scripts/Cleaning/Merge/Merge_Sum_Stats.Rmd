...
We merge on last name, state, and the first two letters of the first name. We have `r length(unique(patent$unique_inventor_id))` unique inventors that we are trying to find matches for. The corresponding infutor set has `r length(unique(infutor$pid))` unique person ids. 
